...
def test_agent(env, model, start_time, episode_length, warmup_period,
               log_dir=os.getcwd(), model_name='test', 
               save_to_file=True, plot=False):
...
def plot_results(env, rewards, points=['reaTZon_y','reaTSetHea_y','reaTSetCoo_y','oveHeaPumY_u',
    ...
    # From the df get the time and 'oveHeaPumY_u' columns as another df called actions_df
    actions_df = df[['time', 'oveHeaPumY_u']]
    # Save actions_df to a csv file
